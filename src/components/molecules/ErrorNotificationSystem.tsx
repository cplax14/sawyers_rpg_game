...
export const ErrorNotificationSystem: React.FC<ErrorNotificationSystemProps> = ({
  ...
  const { isRecoverable, getRecoverySuggestions, executeWithRecovery, getSystemStatus } =
  ...
  const retryOperation = useCallback(
    async (notification: ErrorNotification) => {
      ...
      try {
        // Update retry count
        setNotifications(prev =>
          prev.map(n => (n.id === notification.id ? { ...n, retryCount: n.retryCount + 1 } : n))
        );
    ...
    >
      <AnimatePresence>
        {visibleNotifications.map(notification => {
          ...
          const canRetry =
            notification.retryCount < notification.maxRetries && isRecoverable(notification.error);
                  ...
                  {isExpanded && (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {getPriorityRecoveryActions(notification.error, 4).map(
                            (action, index) => (
