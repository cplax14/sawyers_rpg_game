  ...
  return (
    <div className="relative">
      <div className="flex justify-between text-xs text-gray-400 mb-1">
        <span>{label}</span>
        <span>
          {data.length > 0 ? `${data[data.length - 1].toFixed(1)}${unit}` : '--'}
        </span>
      </div>
      <canvas
        ref={canvasRef}
            ...
            <div className="mx-3 mb-2 p-2 bg-yellow-900 border border-yellow-600 rounded text-xs">
              ...
              {pauseReason.metrics && (
                <div className="text-yellow-400 mt-1">
                  FPS: {pauseReason.metrics.fps} |
                  Save Time: {pauseReason.metrics.serializationTime.toFixed(1)}ms
                </div>
              )}
            </div>
          )}
                ...
                <div className="p-3 border-t border-gray-700">
                  ...
                  <div className="mt-4 p-2 bg-gray-800 rounded text-xs">
                    ...
                    <div className="space-y-1 text-gray-300">
                      <div className="flex justify-between">
                        <span>Active:</span>
                        <span className={autoSaveState.isActive ? 'text-green-400' : 'text-red-400'}>
                          {autoSaveState.isActive ? 'Yes' : 'No'}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Paused:</span>
                        <span className={autoSaveState.isPaused ? 'text-yellow-400' : 'text-green-400'}>
