...
export const StatComparison: React.FC<StatComparisonProps> = ({
  ...
  const statChanges = useMemo((): StatChange[] => {
    ...
    if (currentItem?.statModifiers) {
      Object.keys(currentItem.statModifiers).forEach(stat => allStats.add(stat as keyof PlayerStats));
    }
    ...
    allStats.forEach(stat => {
      ...
      const absoluteChange = Math.abs(change);
      const significance =
        absoluteChange >= 10 || Math.abs(percentage) >= 50 ? 'major' :
        absoluteChange >= 5 || Math.abs(percentage) >= 20 ? 'significant' :
        change !== 0 ? 'minor' : 'none';
  ...
  const relevantChanges = useMemo(() => {
    return highlightChanges
      ? statChanges.filter(change => change.change !== 0)
      : statChanges;
  ...
  return (
    ...
    >
      ...
      <div style={comparisonStyles.header}>
        ...
        {!compact && (
          <p style={comparisonStyles.subtitle}>
            Compare current equipment with new item
          </p>
        )}
      </div>
      ...
      <div style={{
        ...
        <motion.div
          style={{
            ...
            ...(currentItem ? { borderLeft: `3px solid ${getRarityColor(currentItem.rarity)}` } : {})
          }}
        ...
        >
          <div style={comparisonStyles.itemHeader}>
            ...
            {currentItem && (
              <p style={comparisonStyles.itemRarity}>
                {currentItem.rarity}
              </p>
            )}
          </div>
          ...
          <div style={{
            ...
            {statChanges.map(({ stat, current }) => (
              ...
              >
                <span style={comparisonStyles.statLabel}>
                  {formatStatName(stat)}:
                </span>
      ...
      {showNetChange && (
        ...
        >
          ...
          {hasChanges ? (
            relevantChanges.map(({ stat, change, isImprovement, percentage, significance }, index) => {
              const baseValue = baseStats[stat] || 10;
              ...
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    ...(change > 0 ? comparisonStyles.changeImprovement :
                        change < 0 ? comparisonStyles.changeDegrade :
                    ...
                    {indicator.badge && (
                      <motion.span
                        style={{
                          ...
                          ...(indicator.badge === 'major_upgrade' ? comparisonStyles.majorUpgradeBadge :
                              indicator.badge === 'minor_upgrade' ? comparisonStyles.minorUpgradeBadge :
                              indicator.badge === 'major_downgrade' ? comparisonStyles.majorDowngradeBadge :
            ...
            <div style={comparisonStyles.noChangeText}>
              No stat changes
            </div>
          )}
          ...
          {hasChanges && (
            ...
            >
              <div style={{
                ...
                Total Change: {relevantChanges.reduce((sum, change) => sum + change.change, 0) > 0 ? '+' : ''}
              ...
              <div style={comparisonStyles.totalStatLabel}>
                Overall Equipment Impact
              </div>
            </motion.div>
          )}
        </motion.div>
