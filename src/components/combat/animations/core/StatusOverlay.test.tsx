...
describe('StatusOverlay', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render when isActive is true', () => {
      const { container } = render(<StatusOverlay {...defaultProps} isActive={true} />);
      expect(container.firstChild).toBeInTheDocument();
    });

    it('should not render when isActive is false', () => {
      const { container } = render(<StatusOverlay {...defaultProps} isActive={false} />);
  ...
  describe('Status Type Variations', () => {
    it('should render with poison status type', () => {
      const { container } = render(<StatusOverlay {...defaultProps} statusType='poison' />);
      expect(container).toHaveTextContent('☠️');
    });

    it('should render with sleep status type', () => {
      const { container } = render(<StatusOverlay {...defaultProps} statusType='sleep' />);
      expect(container).toHaveTextContent('💤');
    });

    it('should render with silence status type', () => {
      const { container } = render(<StatusOverlay {...defaultProps} statusType='silence' />);
      expect(container).toHaveTextContent('🔇');
    });

    it('should render with slow status type', () => {
      const { container } = render(<StatusOverlay {...defaultProps} statusType='slow' />);
      expect(container).toHaveTextContent('🐌');
    });

    it('should render with stun status type', () => {
      const { container } = render(<StatusOverlay {...defaultProps} statusType='stun' />);
      expect(container).toHaveTextContent('⭐');
    });

    it('should render with burn status type', () => {
      const { container } = render(<StatusOverlay {...defaultProps} statusType='burn' />);
      expect(container).toHaveTextContent('🔥');
    });

    it('should render with freeze status type', () => {
      const { container } = render(<StatusOverlay {...defaultProps} statusType='freeze' />);
      expect(container).toHaveTextContent('❄️');
    });
  });
  ...
  describe('Prop Validation', () => {
    it('should handle different colors', () => {
      const color = '#ff0000';
      const { container } = render(<StatusOverlay {...defaultProps} color={color} />);
      expect(container.firstChild).toBeInTheDocument();
    });

    it('should handle custom intensity', () => {
      const { container } = render(<StatusOverlay {...defaultProps} intensity={0.8} />);
      expect(container.firstChild).toBeInTheDocument();
    });
    ...
    it('should handle different positions', () => {
      const { container } = render(<StatusOverlay {...defaultProps} targetX={300} targetY={400} />);
      const wrapper = container.querySelector('div');
  ...
  describe('Edge Cases', () => {
    it('should handle negative coordinates', () => {
      const { container } = render(<StatusOverlay {...defaultProps} targetX={-50} targetY={-50} />);
      const wrapper = container.querySelector('div');
    ...
    it('should handle zero intensity', () => {
      const { container } = render(<StatusOverlay {...defaultProps} intensity={0} />);
      expect(container.firstChild).toBeInTheDocument();
    });

    it('should handle intensity greater than 1', () => {
      const { container } = render(<StatusOverlay {...defaultProps} intensity={1.5} />);
      expect(container.firstChild).toBeInTheDocument();
    });

    it('should handle toggling isActive', () => {
      const { container, rerender } = render(<StatusOverlay {...defaultProps} isActive={true} />);
      expect(container.firstChild).toBeInTheDocument();
