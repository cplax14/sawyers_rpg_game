...
import { CriticalImpactRings, CriticalIndicator } from '../CriticalHitEffects';
      ...
      >
          ...
          <>
            ...
            {meteorImpacts.map((impact, index) => (
              <motion.div
                ...
                onAnimationComplete={
                  index === meteorImpacts.length - 1 ? handleWarningComplete : undefined
                }
                style={{
                  position: 'absolute',
                  left: impact.x - 45,
        ...
        {phase === 'impact' && (
          <>
            ...
            {meteorImpacts.map((impact, index) => (
              <React.Fragment key={`meteor-${index}`}>
                ...
                <motion.div
                  ...
                  onAnimationComplete={
                    index === meteorImpacts.length - 1 ? handleImpactComplete : undefined
                  }
                  style={{
                    position: 'absolute',
                    left: impact.x - 12,
                ...
                {/* Impact explosion - enhanced for critical hits */}
                <motion.div
                  initial={{ opacity: 0, scale: 0 }}
                  animate={{
                    opacity: [0, 1 * critMultiplier.glowOpacity, 0.8, 0],
                    scale: [
                      0,
                  ...
                  style={{
                    ...
                    background: isCritical
                      ? `radial-gradient(circle, #ffd700 0%, ${FIRE_COLORS.accent}ff 20%, ${FIRE_COLORS.primary}dd 50%, ${FIRE_COLORS.secondary}80 70%, transparent 90%)`
                      : `radial-gradient(circle, ${FIRE_COLORS.accent}ff 0%, ${FIRE_COLORS.primary}dd 40%, ${FIRE_COLORS.secondary}80 70%, transparent 90%)`,
                    filter: `blur(${isCritical ? 12 : 10}px)`,
                  }}
                />
                ...
                {/* Explosion particles - more for critical hits */}
                {validateParticleCount(
                  Math.floor(12 * Math.min(critMultiplier.particleCount, 30 / (12 * 3))), // Divide by 3 meteors
                  'MeteorAnimation',
                  `impact-meteor-${index}`
                )}
                <ParticleSystem
                  ...
                  particleCount={Math.floor(
                    12 * Math.min(critMultiplier.particleCount, 30 / (12 * 3))
                  )}
            ...
            {/* Screen flash effect (red/orange) - enhanced for critical hits */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{
                opacity: [0, 0.25 * critMultiplier.screenFlash, 0.15, 0],
                transition: {
                  duration: IMPACT_DURATION / 1000,
                  ease: 'easeInOut',
                },
            ...
            {/* Custom meteor screen shake (stronger 6px for meteor) */}
            {isCritical && (
              <motion.div
                animate={{
          ...
          <>
            ...
            {meteorImpacts.map((impact, index) => (
              <React.Fragment key={`dust-${index}`}>
                ...
                <motion.div
                  ...
                  onAnimationComplete={
                    index === meteorImpacts.length - 1 ? handleAftermathComplete : undefined
                  }
                  style={{
                    position: 'absolute',
                    left: impact.x - 40,
            ...
            <motion.div
              ...
              style={{
                ...
                borderRadius: '50%',
                background:
                  'radial-gradient(circle, rgba(60,60,60,0.4) 0%, rgba(40,40,40,0.2) 50%, transparent 70%)',
                filter: 'blur(20px)',
              }}
            />
          </>
