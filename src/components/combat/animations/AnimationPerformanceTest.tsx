...
export const AnimationPerformanceTest: React.FC = () => {
  ...
  const triggerAnimation = (spell: TestSpell, isCritical: boolean) => {
    console.log(`🎬 [Performance Test] Casting ${spell.name} (${isCritical ? 'CRITICAL' : 'normal'})`);
  ...
  const handleAnimationComplete = useCallback(() => {
    ...
    if (nextIndex < testQueue.current.length) {
      ...
      }, testMode === 'stress' ? 100 : 500); // Faster in stress test mode
    } else {
      // Test complete
      completeTest();
    }
  ...
  const generateReport = (testEnd: number) => {
    ...
    if (overallFPS.average < 55) {
      failureReasons.push(`Average FPS ${overallFPS.average.toFixed(1)} below target (55fps minimum)`);
      passed = false;
    }
    ...
    console.log(`   Overall FPS: ${overallFPS.average.toFixed(1)} (min: ${overallFPS.min.toFixed(1)}, max: ${overallFPS.max.toFixed(1)})`);
      ...
      {isRunning && currentTest && (
        <div style={{
          ...
          <p style={{ margin: '0', fontSize: '18px' }}>
            <strong>{currentTest.spell.name}</strong> {currentTest.isCritical ? '(CRITICAL HIT)' : '(Normal Hit)'}
          </p>
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
              <StatCard label="Test Duration" value={`${(report.totalDuration / 1000).toFixed(2)}s`} />
              ...
              <StatCard
                ...
                status={report.overallFPS.frameDrops / report.overallFPS.samples.length < 0.1 ? 'good' : 'bad'}
              />
            </div>
          </div>
