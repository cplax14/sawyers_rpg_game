...
interface UseDataIntegrityResult {
  ...
  validateCloudDownload: (data: any, expectedChecksum?: string) => Promise<DataIntegrityResult>;
...
export const useDataIntegrity = (options: UseDataIntegrityOptions = {}): UseDataIntegrityResult => {
  const {
  ...
  const updateMetrics = useCallback(
    (isSuccessful: boolean, isCorrupted: boolean, isRecovered: boolean, validationTime: number) => {
   ...
   */
  const validateWithChecksum = useCallback(
    async (
      gameState: ReactGameState,
      expectedChecksum: string,
      options: IntegrityValidationOptions = {}
   ...
   */
  const verifyDataChecksum = useCallback(
    async (data: any, expectedChecksum: string): Promise<boolean> => {
      try {
   ...
   */
  const prepareForCloudUpload = useCallback(
    async (
      gameState: ReactGameState
    ): Promise<{
      data: ReactGameState;
   ...
   */
  const validateCloudDownload = useCallback(
    async (data: any, expectedChecksum?: string): Promise<DataIntegrityResult> => {
      try {
