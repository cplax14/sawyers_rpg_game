...
const convertToEnhancedItem = (reactItem: ReactItem): EnhancedItem => {
  ...
  return {
    ...reactItem,
    category:
      reactItem.type === 'consumable'
        ? 'consumables'
        : reactItem.type === 'material'
          ? 'materials'
          : reactItem.type === 'quest'
            ? 'quest'
            : 'equipment',
    // Preserve explicit equipmentSlot if provided, otherwise infer from type
    equipmentSlot: hasExplicitSlot
      ? (reactItem as any).equipmentSlot
      : reactItem.type === 'weapon'
        ? 'weapon'
        : reactItem.type === 'armor'
          ? 'armor'
          : reactItem.type === 'accessory'
            ? 'accessory'
            : undefined,
...
export const useInventory = () => {
    ...
    return {
      ...
      metadata: {
        ...
        totalValue: inventoryState.containers.main.items.reduce(
          (sum, slot) => sum + (slot.item?.value || 0) * slot.quantity,
          0
        ),
  ...
  const addItem = useCallback(
    async (
      ...
      try {
        ...
        const nonEquippedItemCount = container.items.filter(
          slot => slot.item !== null && !isItemEquipped(slot.item.id)
        ).length;
  ...
  const useItem = useCallback(
    async (itemId: string, quantity: number = 1): Promise<UseItemResult> => {
      try {
        ...
        return {
          ...
          effects: [],
          message:
            error instanceof Error ? error.message : 'Unknown error occurred while using item',
        };
      }
    },
  ...
  const consolidateInventoryStacks = useCallback(
    async (containerId: string = 'main'): Promise<void> => {
      const container = inventoryState.containers[containerId];
      if (!container) return;

      // Extract items from slots
      const items = container.items.filter(slot => slot.item !== null).map(slot => slot.item!);
  ...
  const addItemWithStacking = useCallback(
    async (
      ...
      const existingSlot = container.items.find(
        slot =>
          slot.item &&
  ...
  const markEquippedItems = useCallback(
    (slots: InventorySlot[]): InventorySlot[] => {
  ...
  const getFilteredItems = useCallback(
    (containerId: string = 'main', filter?: Partial<InventoryFilter>): InventorySlot[] => {
      const container = inventoryState.containers[containerId];
      if (!container) return [];

      const activeFilter: InventoryFilter = filter
      ...
      if (activeFilter.categories.length > 0) {
        items = items.filter(
          slot => slot.item && activeFilter.categories.includes(slot.item.category)
        );
      }

      if (activeFilter.rarities.length > 0) {
        items = items.filter(slot => slot.item && activeFilter.rarities.includes(slot.item.rarity));
      }
  ...
  const getItemsByCategory = useCallback(
    (category: string, containerId: string = 'main'): EnhancedItem[] => {
      const container = inventoryState.containers[containerId];
      if (!container) return [];
  ...
  const searchItems = useCallback(
    (query: string, items?: EnhancedItem[]): EnhancedItem[] => {
      const searchTarget =
        items ||
        getFilteredItems()
          .map(slot => slot.item!)
  ...
  const getTotalItemCount = useCallback(
    (containerId: string = 'main'): number => {
      const container = inventoryState.containers[containerId];
  ...
  const updateFilter = useCallback(
    (containerId: string = 'main', filter: Partial<InventoryFilter>) => {
      setInventoryState(prev => ({
        ...prev,
        containers: {
          ...prev.containers,
  ...
  // Update sort option
  const updateSort = useCallback((containerId: string = 'main', sortBy: InventorySortOption) => {
    setInventoryState(prev => ({
      ...prev,
      containers: {
        ...prev.containers,
      ...
      totalValue: Object.values(inventoryState.containers).reduce(
        (sum, container) =>
          ...
          container.items.reduce(
            (containerSum, slot) => containerSum + (slot.item?.value || 0) * slot.quantity,
            0
          ),
        0
      ),
