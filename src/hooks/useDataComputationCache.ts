...
export function useDataComputationCache() {
  ...
  const computeWithCache = useCallback(async <T>(
    cacheKey: string,
        ...
        if (items.length > 1000) {
          const filterBatch = async (batch: EnhancedItem[]) => {
            return batch.filter(item => {
              ...
              // Rarity filter
              if (filters.rarity && filters.rarity.length > 0 && !filters.rarity.includes(item.rarity)) {
                return false;
              }

              // Value range filter
              if (filters.valueRange) {
                if (filters.valueRange.min && item.value < parseInt(filters.valueRange.min.toString())) {
                  return false;
                }
                if (filters.valueRange.max && item.value > parseInt(filters.valueRange.max.toString())) {
                  return false;
                }
              }
...
export function useSmartCreatureFiltering(creatures: EnhancedCreature[]) {
  ...
  const filterCreatures = useCallback(async (
    filters: {
    ...
    return computeWithCache(
      ...
      async () => {
        ...
        filtered = filtered.filter(creature => {
          ...
          // Rarity filter
          if (filters.rarity && filters.rarity.length > 0 && !filters.rarity.includes(creature.rarity)) {
            return false;
          }
          ...
          if (filters.search && filters.search.trim()) {
            const searchTerm = filters.search.toLowerCase();
            if (!creature.name.toLowerCase().includes(searchTerm) &&
...
export function useSmartAggregations() {
  ...
  const computeItemStats = useCallback(async (items: EnhancedItem[]) => {
    return computeWithCache(
      'item-stats',
      () => {
        const stats = {
  ...
  const computeCreatureStats = useCallback(async (creatures: EnhancedCreature[]) => {
    return computeWithCache(
      'creature-stats',
      () => {
        const stats = {
        ...
        for (const creature of creatures) {
          ...
          const power = creature.stats.attack + creature.stats.defense +
                       (creature.stats.health || creature.stats.hp || 0) + (creature.stats.speed || 0);
          totalPower += power;
          ...
          stats.statusDistribution[statusKey] =
            (stats.statusDistribution[statusKey] || 0) + 1;
        }
