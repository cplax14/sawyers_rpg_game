...
export class SaveSystemManager {
   ...
   */
  async loadGame(options: LoadOperationOptions): Promise<SaveOperationResult<SaveGameData>> {
    if (!this.isInitialized) {
      return {
        success: false,
        error: new Error('Save system not initialized'),
    ...
    try {
      ...
      // Load from IndexedDB
      const loadResult = await this.indexedDbManager.loadFromSlot(slotNumber, (progress, status) =>
   ...
   */
  async importSave(file: File, targetSlot: number): Promise<SaveImportResult> {
    try {
  ...
  async updateSyncStatus(
    ...
    return await this.indexedDbManager.updateSyncStatus(
      slotNumber,
      status,
      isCloudAvailable,
      lastError
  ...
  private generateSaveMetadata(
    ...
    const validSlotNumber =
      typeof slotNumber === 'number' && !isNaN(slotNumber) && isFinite(slotNumber) ? slotNumber : 0; // Default to slot 0
  ...
  private generatePlayerSummary(gameState: ReactGameState): SavePlayerSummary {
    ...
    return {
      ...
      achievementsUnlocked:
        gameState.experience?.achievements?.filter(a => a.completed).length || 0,
    };
  }
  ...
  private isLegacyFormat(data: any): boolean {
    ...
    return data && typeof data === 'object' && data.player && !data.metadata;
  }
