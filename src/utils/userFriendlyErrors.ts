...
const ERROR_MESSAGE_DATABASE: Record<CloudErrorCode, UserFriendlyErrorData> = {
  [CloudErrorCode.NETWORK_UNAVAILABLE]: {
    ...
    message: "Your device isn't connected to the internet, so cloud features aren't available right now.",
    explanation: "Cloud save features require an active internet connection to sync your game progress. Without internet, you can continue playing, but your progress will only be saved locally.",
    immediate_actions: [
    ...
    when_to_contact_support: "If your internet works for other apps but cloud save consistently fails",
  ...
  [CloudErrorCode.NETWORK_ERROR]: {
    ...
    explanation: "This usually happens due to temporary network problems, server maintenance, or high traffic. Your local progress is safe.",
    immediate_actions: [
  ...
  [CloudErrorCode.NETWORK_TIMEOUT]: {
    ...
    explanation: "This happens when your internet connection is very slow or unstable. The operation was safely cancelled to protect your save data.",
    immediate_actions: [
  ...
  [CloudErrorCode.AUTH_REQUIRED]: {
    ...
    explanation: "Cloud saves are linked to your account for security and privacy. Signing in ensures your saves are protected and accessible only by you.",
    immediate_actions: [
  ...
  [CloudErrorCode.AUTH_EXPIRED]: {
    ...
    explanation: "For your security, login sessions automatically expire after a period of time. This helps protect your account from unauthorized access.",
    immediate_actions: [
  ...
  [CloudErrorCode.AUTH_INVALID]: {
    ...
    explanation: "The login credentials don't match any account in our system. This could be due to typos, forgotten passwords, or using the wrong email address.",
    immediate_actions: [
    ...
    when_to_contact_support: "If you're certain your credentials are correct but still can't sign in",
  ...
  [CloudErrorCode.STORAGE_QUOTA_EXCEEDED]: {
    ...
    explanation: "Each account has a limited amount of cloud storage space. When it's full, you can't save new data until you delete some old saves or upgrade your storage.",
    immediate_actions: [
  ...
  [CloudErrorCode.STORAGE_PERMISSION_DENIED]: {
    ...
    explanation: "This error occurs when there's a problem with your account permissions or if you're trying to access data that doesn't belong to your account.",
    immediate_actions: [
  ...
  [CloudErrorCode.STORAGE_NOT_FOUND]: {
    ...
    explanation: "This save file may have been deleted, corrupted, or moved. It could also be a temporary server issue preventing access to your data.",
    immediate_actions: [
  ...
  [CloudErrorCode.STORAGE_CORRUPTED]: {
    ...
    explanation: "Save file corruption can happen due to interrupted uploads, storage device issues, or data transmission errors. The file cannot be safely loaded to prevent further issues.",
    immediate_actions: [
  ...
  [CloudErrorCode.DATA_TOO_LARGE]: {
    ...
    explanation: "Save files have size limits to ensure good performance and reasonable storage usage. Your save file has exceeded this limit, possibly due to large inventory or extensive progress.",
    immediate_actions: [
  ...
  [CloudErrorCode.DATA_INVALID]: {
    ...
    explanation: "This can happen if the save file was modified, corrupted, or created by a different version of the game. The system cannot safely process this data.",
    immediate_actions: [
      "Try loading a different save file",
  ...
  [CloudErrorCode.DATA_CORRUPTED]: {
    ...
    explanation: "Data corruption can occur due to storage device issues, interrupted transfers, or system problems. The corrupted data cannot be safely used.",
    immediate_actions: [
  ...
  [CloudErrorCode.DATA_CHECKSUM_MISMATCH]: {
    ...
    explanation: "Every save file has a security signature to ensure it hasn't been modified inappropriately. This file's signature doesn't match, so it cannot be loaded for security reasons.",
    immediate_actions: [
  ...
  [CloudErrorCode.DATA_VERSION_CONFLICT]: {
    ...
    explanation: "The save file format has changed between game versions. This save file was created with a newer or incompatible version and cannot be loaded safely.",
    immediate_actions: [
  ...
  [CloudErrorCode.SYNC_CONFLICT]: {
    ...
    explanation: "This happens when the same save slot has been modified on different devices or sessions. You need to choose which version to keep.",
    immediate_actions: [
  ...
  [CloudErrorCode.SYNC_INTERRUPTED]: {
    ...
    explanation: "The sync process was stopped due to connection issues, device sleep, or other interruptions. The operation needs to be completed to ensure your data is safely stored.",
    immediate_actions: [
      "Check your internet connection",
  ...
  [CloudErrorCode.SYNC_PARTIAL_FAILURE]: {
    ...
    explanation: "This mixed result typically happens due to network issues, file size problems, or server capacity limits affecting some files but not others.",
    immediate_actions: [
  ...
  [CloudErrorCode.OPERATION_CANCELLED]: {
    ...
    explanation: "This can happen if you cancelled the operation manually, if the app was closed, or if the system cancelled it due to resource constraints.",
    immediate_actions: [
  ...
  [CloudErrorCode.OPERATION_TIMEOUT]: {
    ...
    explanation: "To prevent indefinite waiting and potential data issues, operations are cancelled if they take too long. This usually indicates connection or server performance issues.",
    immediate_actions: [
  ...
  [CloudErrorCode.OPERATION_FAILED]: {
    ...
    explanation: "This is a general error that can have various causes including server issues, corrupted data, or system problems. More specific information may be available in the error details.",
    immediate_actions: [
      "Try the operation again",
  ...
  [CloudErrorCode.CONFIG_INVALID]: {
    ...
    message: "There's a problem with the game's configuration that prevents cloud features from working.",
    explanation: "The game's settings or configuration files have invalid or missing information required for cloud operations. This typically requires technical intervention.",
    immediate_actions: [
  ...
  [CloudErrorCode.CONFIG_MISSING]: {
    ...
    explanation: "Essential configuration files that enable cloud features are missing. This can happen due to incomplete installation, file corruption, or accidental deletion.",
    immediate_actions: [
  ...
  [CloudErrorCode.UNKNOWN]: {
    ...
    explanation: "This error doesn't match any known issues. It could be due to new problems, unique system configurations, or rare edge cases that need investigation.",
    immediate_actions: [
      "Try the operation again",
      "Restart the game",
  ...
  [CloudErrorCode.INTERNAL]: {
    ...
    message: "There's an internal problem with our systems that's preventing the operation from completing.",
    explanation: "This indicates a problem on our end rather than with your device or connection. Our technical team needs to address this server-side issue.",
    immediate_actions: [
      "Wait a few minutes and try again",
 ...
 */
export function getContextualErrorMessage(error: CloudError, context?: {
  isFirstTime?: boolean;
  ...
  if (context) {
    if (context.isFirstTime && error.code === CloudErrorCode.AUTH_REQUIRED) {
      message = "Welcome! To use cloud saves and sync your progress across devices, you'll need to create a free account. This keeps your saves secure and accessible from anywhere.";
    }

    if (context.hasLocalSaves && (error.code === CloudErrorCode.NETWORK_UNAVAILABLE || error.code === CloudErrorCode.NETWORK_ERROR)) {
    ...
    if (context.operationType === 'save' && error.code === CloudErrorCode.STORAGE_QUOTA_EXCEEDED) {
      message = "Your cloud storage is full. Before saving this progress, you'll need to delete some older saves to make room.";
    }
  }
 ...
 */
export function shouldContactSupport(error: CloudError, hasTriedBasicSteps: boolean = false): {
 ...
 */
export function generateTroubleshootingGuide(error: CloudError, context?: {
  isFirstTime?: boolean;
  hasLocalSaves?: boolean;
  isOnMobile?: boolean;
  operationType?: 'save' | 'load' | 'sync' | 'delete';
