...
class BaseUIModule {
    ...
    attachButton(buttonId, callback, options = {}) {
        // In React environments, buttons may not be immediately available
        // Set required: false by default to reduce warning noise
        const defaultOptions = { required: false, ...options };

        if (this.uiManager && typeof this.uiManager.attachButton === 'function') {
            return this.uiManager.attachButton(buttonId, callback, defaultOptions);
        }
        if (window.uiHelpers && typeof window.uiHelpers.attachButton === 'function') {
            return window.uiHelpers.attachButton(buttonId, callback, defaultOptions);
        }
        ...
        if (defaultOptions.required !== false) {
